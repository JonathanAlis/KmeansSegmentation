...
cout	<<src.rows<<endl;
	...
	interest[0]=Rect(100,50,2300,300);//fundo
	interest[1]=Rect(2000,1650,400,300);//chao
	interest[2]=Rect(1400,400,280,450);//fruta vermelha
	interest[3]=Rect(720,500,200,400);//fruta verde
	interest[4]=Rect(1750,420,400,400);//frutas amareladas
	interest[5]=Rect(800,1100,500,500);//cesta e madeira
	interest[6]=Rect(1320,1530,600,100);//alho
	interest[7]=Rect(1220,1850,250,100);//pano
	interest[8]=Rect(600,1730,300,100);//saca rolhas
	...
	for (int i=0;i<num_classes;i++){
		Point p1=Point(rnd.uniform(0,src.cols-1),rnd.uniform(0,src.rows-1)-100);
		Point p2=Point(rnd.uniform(0,src.cols-1),rnd.uniform(0,src.rows-1)-100);
		int x, w;
	...
	imwrite("./clustering_random/interest_points_fruit.png",show);
	...
	out << iteracao;
	s = out.str();
	imwrite("./clustering_random/fruit_BGR_step_"+s+".png",colorSegBGR);
...
vector<Scalar> centroideHSV_novo(num_classes);
	...
	out << iteracao;
	s = out.str();
	imwrite("./clustering_random/fruit_HSV_step_"+s+".png",colorSegHSV);
	waitKey(100);
