...
vector<Scalar> color(num_classes);
	...
	namedWindow("resultado por hue", WINDOW_NORMAL);
	imshow("resultado por hue",dst);
	
	imwrite("./resultado_regioes/fruit_regioes_hue.png",dst);
...
dst=Scalar(0);
	...
	imwrite("./resultado_regioes/fruit_regioes_value.png",dst);
