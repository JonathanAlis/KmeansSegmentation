...
cout	<<src.rows<<endl;
	...
	imwrite("./clustering_chosen/interest_points_wine.png",show);
	//BGR
	...
	out << iteracao;
	s = out.str();
	imwrite("./clustering_chosen/wine_BGR_step_"+s+".png",colorSegBGR);
...
vector<Scalar> centroideHSV_novo(num_classes);
	...
	out << iteracao;
	s = out.str();
	imwrite("./clustering_chosen/wine_HSV_step_"+s+".png",colorSegHSV);
	waitKey(100);
